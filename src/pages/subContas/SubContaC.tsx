...
const SubContaC = () => {
  ...
  const confirmDelete = () => {
    if (selectedSubContaIndex !== null) {
      dispatch(
        deleteSubConta({ subConta: deleteSub, id: selectedSubContaIndex })
      )
        .then(() => {
          ...
          {currentSubContas.map((subC, index) => (
            <tr key={index}>
              ...
              <td>
                <p
                  style={{
                    backgroundColor: subC.tipo === "FIXA" ? " rgba(255, 69, 0, 0.05)" : "rgba(0, 255, 255, 0.05)",
                    width: "160px",
                    padding: '5px',
                    borderRadius: "7px",
                    margin: "0 auto",
                    ...
                    color: subC.tipo === "FIXA" ? " rgba(255, 69, 0, 1)" : "rgba(0, 255, 255, 1)",
                  }}
                >
                  {subC.tipo === "FIXA" ? "DESPESA FIXA" : "DESPESA VARIÁVEL"}
                </p>
              </td>
              <td>
                <button onClick={() => { openModal(true, (currentPage - 1) * itemsPerPage + index); setNewSubCont(subC.nome) }}>
                  <FaPencilAlt />
                </button>
              </td>
              <td>
                <button onClick={() => { handleDeleteConfirmation((currentPage - 1) * itemsPerPage + index); setDeleteSub(subC.nome) }}>
                  <AiTwotoneDelete />
                </button>
              </td>
            </tr>
